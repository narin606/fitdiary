...
PORT=4402 node dist/analysisProvider.integration-server.js > /tmp/fitdiary-integration-provider-$$.log 2>&1 & PROVIDER_PID=$!
for _ in {1..60}; do curl --fail --silent "http://127.0.0.1:4402/count" >/dev/null 2>&1 && break; sleep 0.1; done
NODE_ENV=test PORT="$API_PORT" FRONTEND_URL="http://127.0.0.1:3401" DATABASE_URL="$TEST_DATABASE_URL" \
UPLOAD_DIR="$UPLOAD_DIR" \
AI_BASE_URL="http://127.0.0.1:4402/v1" AI_API_KEY="integration-key" AI_VISION_MODEL="integration-vision" \
