...
type AuthValue = { user: User | null; loading: boolean; login(identity: string, password: string): Promise<User>; register(username: string, email: string, password: string): Promise<string>; logout(): Promise<void>; refresh(): Promise<void> };
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useCallback(async (identity: string, password: string) => { setLoading(true); try { const next = (await authApi.login(identity, password)).user; setUser(next); return next; } finally { setLoading(false); } }, []);
