...
- **Modern food, progress, and settings experiences.** Foods now uses a visual nutrition library with search and macro cards, Progress includes summary metrics and a weight-trend chart, and Settings groups account details, nutrition goals, hydration, and the daily plan into a clearer responsive layout.
- **Modern meal-grouped diary.** The diary now presents breakfast, lunch, dinner, and snacks as visual sections with a calorie-remaining summary, nutrition details, focused add/edit controls, and useful empty states.
- **Private meal photos.** The Diary makes camera capture and library upload first-class actions. A diary entry can carry a meal photo that is uploaded, viewed, replaced, and deleted only by its owner. Photos are capped at 5 MiB and must be a genuine JPEG, PNG, or WebP by both declared type and file signature. Files are stored outside any public path, under a per-user directory with randomly generated names and restrictive permissions. Retrieval is owner-checked and served `private, no-store`, and no durable public photo URL is issued. Deleting a diary entry also deletes its photo.
...
- **First-run setup.** A new account is now asked for what a calorie budget actually depends on — goal, sex, birthdate, height, current and goal weight, activity level, and weekly pace — instead of starting on default targets. The answers produce daily calorie and macro targets that are shown before saving and can be edited. Targets are calculated server-side with the Mifflin-St Jeor equation and an activity multiplier, adjusted by the chosen weekly pace, and limited by a safe rate of change and a calorie floor; if a requested pace is too aggressive the number is reduced to a safe one and the reason is shown. The starting weight is recorded as the first weigh-in so progress begins populated, setup can be reopened from Settings to correct any answer, and an account that has not finished setup is returned to it rather than shown an empty dashboard.
